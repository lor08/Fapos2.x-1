 ...
 *
 */
class QuestionEntity extends FpsEntity
{
	...
	public function save()
	{
		...
		return ($Register['DB']->save('question', $params));
	}
	...
	{ 
		...
		$attachesModel = $Register['ModManager']->getModelInstance('QuestionAttaches');
		$commentsModel = $Register['ModManager']->getModelInstance('Comments');
		$addContentModel = $Register['ModManager']->getModelInstance('QuestionAddContent');
		...
		$Register['DB']->delete('question', array('id' => $this->id));
	}
	...
	public function getCategory()
	{
        if (!$this->checkProperty($this->category)) {
			if (!$this->getCategory_id()) {
                $Register = Register::getInstance();
				$this->category = $Register['ModManager']->getEntityInstance('QuestionSections');
			} else {
                $Register = Register::getInstance();
				$catsModel = $Register['ModManager']->getModelInstance('QuestionSections');
